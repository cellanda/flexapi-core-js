...
function PropertyBuilder(mapper, settings) {
    ...
    function _PropertyBuilder (_property) {
        ...
        var _public = {
            ...
            get: function () {
                var fnGet = arguments[0];
                var argsGet = Array.prototype.slice.call(arguments, 1);
                var propertyClone = _property.clone();
                propertyClone.selector.addGet('', fnGet, argsGet);
        ...
        var _xpublic = {
            ...
            get: function (fnGet, argsGet) {
                _property.selector.addGet('', fnGet, argsGet);
        ...
        if (settings && settings.getters) {
            __.each(settings.getters, function (parameters, getterName) {
                _public.get[getterName] = function () {
                    var argsGet = Array.prototype.slice.call(arguments, 0);
                    if (parameters.isUnique) {
                        _property.selector.addGet(getterName, parameters.fnGet, argsGet, 0);
                        return clone()._interfaceBasic;
                    }
                    else {
                        _property.selector.addGet(getterName, parameters.fnGet, argsGet);
                        return clone()._interfaceWithIndexParameter;
                    }
                };
            });
