...
function Selector() {
    ...
    function determineIsArray (selector) {
        ...
        for (var step = selector.$firstStep; step; step = step.$next) {
    ...
    function addGet(name, fnGet, args, index) {
        return addStep(this, {
            $type: 'get',
            $name: name,
            $fnGet: fnGet,
    ...
    function get(node, step, isRequired, logger, done) {
        ...
        if (node === null || node === undefined) {
            ...
            try {
                step.$fnGet(node, step.$args, step.$index, logger, function (err, nodes) {
                    if (err) {
    ...
    function evaluate(input, isRequired, logger) {
        ...
        function evaluateStep(input, isArray, step, logger, done) {
            ...
            setTimeout(function () {
                if (step) {
                    if (step.$type === 'get') {
                        logger.pushContextProperty('path', step.$type + (step.$name ? '.' : '') + step.$name + '(\'' + step.$args + '\')');
    ...
    var _public = {
        ...
        addGet: function (name, fnGet, args, index) {
            addGet.call(this, name, fnGet, args, index);
            return this;
        },
